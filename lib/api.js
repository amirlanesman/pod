...
let Client = pm2.Client; // Client is pm2's RPC daemon, we have to use it to get some custom behavior that is not exposed by pm2's CLI

upgradeConf();

// If env var is present, overwrite root dir
// mostly for testing.
if(process.env.POD_ROOT_DIR)
{
...
{
    mkdirp.sync(globalConfig.root);
}

if(!fs.existsSync(globalConfig.root + '/apps'))
{
    fs.mkdirSync(globalConfig.root + '/apps');
}

if(!fs.existsSync(globalConfig.root + '/repos'))
{
...
api.createApp = function(appname, options, callback)
{
    ...
    async.parallel([
        function(done)
        {
            ...
            let data = JSON.stringify(globalConfig, null, 4);
        ...
        if(!err || /is not running/.test(err.toString()))
        {
            async.parallel([
                ...
                function(err)
                {
                    ...
                    return callback(null, 'deleted app: ' + appname.cyan);
                });
        }
        else
        {
...
api.startApp = function(appname, callback)
{
    ...
    fs.exists(app.script, function(exists)
    {
        ...
        Client.executeRemote('getMonitorData', {}, function(err, list)
        {
            ...
            let runningProcs = findInList(appname, list);
            ...
            if(!runningProcs)
            {
                ...
                pm2.start(prepareConfig(app), function(err)
                {
                    ...
                    return callback(null, appname.cyan + ' running on ' + (app.port.yellow || 'unknown port'));
                });
...
api.stopApp = function(appname, callback)
{
    ...
    Client.executeRemote('getMonitorData', {}, function(err, list)
    {
        ...
        if(!runningProcs)
        {
            return callback(null, appname.cyan + ' is not running');
        }
        else
        {
            async.map(runningProcs, function(proc, done)
            {
            ...
            }, function(err)
                {
                    ...
                    let l = runningProcs.length;

                    return callback(
                        null,
                        appname.cyan + ' stopped' +
...
{
    let appList = Object.keys(globalConfig.apps);
...
function restart(app, callback)
{
    Client.executeRemote('restartProcessId', {id: app.pm_id}, function(err)
    {
        ...
        return callback(null, app.pm2_env.name.cyan + ' restarted');
    });
}
...
function createAppRepo(info, done)
{
    async.series([
        ...
        function(next)
        {
            ...
            {
                next(err, 'created bare repo at ' + info.repoPath.cyan);
            });
        },
        function(next)
        {
        ...
        function(next)
        {
            ...
            {
                next(err, 'created empty working copy at ' + info.workPath.cyan);
            });
        }
    ], function(err, msgs)
        {
...
function prepareConfig(appInfo)
{
    ...
    var conf = {
        ...
        env: {
            NODE_ENV: process.env.NODE_ENV || appInfo.config.node_env || globalConfig.node_env || 'development',
    ...
    {
        if(globalConfig.env.hasOwnProperty(o))
        {
...
function upgradeConf()
{
    ...
    function convert(conf)
    {
        for(var key in conf)
        {
            ...
            {
                let converted = fieldsToConvert[key];
