...
function formatAppInfo(app)
{
    ...
    const l = (app.instances && app.instances.length) || 0,
        instances = l > 1 ? (' (' + l + ')') : '',
    ...
    let restarts = countTotalRestarts(app.instances);
    let uptime = formatTime(app.uptime);
    let memory = formatMemory(app.instances);
    let cpu = formatCPU(app.instances);
